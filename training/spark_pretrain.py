...
def main():
    ...
    # Chỉ lấy dataloader cho spark
    _, sparK_dataloader = dataset.Pre_Train_DataLoader()
    print("Dữ liệu huấn luyện đã sẵn sàng.")
    print(f"Số lượng batch: {len(sparK_dataloader)}")

    model_spark = SparK().to(device)
    print(f"Model SparK, kiến trúc {cfg.PRETRAIN_CONFIG['BACKBONE']} đã được khởi tạo")

    criterion = nn.MSELoss().to(device) #loss
    optimizer = torch.optim.SGD( # hàm tối ưu, hội tụ chậm nhưng ổn định
        model_spark.parameters(),
    ...
    if os.path.exists(resume_path):
        ...
        model_spark.load_state_dict(checkpoint['state_dict'])
    ...
    with open(METRICS_CSV_PATH, csv_mode, newline="", encoding="utf-8") as f:
        ...
        for epoch in range(start_epoch, cfg.PRETRAIN_CONFIG["EPOCHS"]):
            ...
            for i, images in enumerate(pbar):
                # Với autoencoder, ảnh đầu vào cũng là ảnh mục tiêu
                ...
                with autocast(device_type="cuda" if device.type == "cuda" else "cpu",enabled=cfg.USE_AMP):
                    output = model_spark(images)
                    loss = criterion(output, images) # Loss là sự khác biệt giữa ảnh tái tạo và ảnh gốc
            ...
            # tính epoch
            avg_loss = total_loss / len(sparK_dataloader)
            ...
            checkpoint_dict = {
                'epoch': epoch + 1,
                'state_dict': model_spark.state_dict(),
            ...
            if (epoch + 1) == cfg.PRETRAIN_CONFIG["EPOCHS"]:
                save_path = os.path.join(cfg.CHECKPOINT_SAVE_PRETRAIN, f"pretrain_spark_epoch_{epoch+1}.pth.tar")
